...
public final class IntersectionCalc2D {
    ...
    public boolean intersects() {
        ...
        direction = new Vector2(a, Vector.NEGATE_BIT);
        loopCount = 0;
        while(true) {
            loopCount++;
            if(loopCount > 200) {
    ...
    private boolean process() {
        Vector2 ao = new Vector2(a, Vector.NEGATE_BIT);
    ...
    private boolean lineCase(Vector2 ao, Vector2 ab) {
        if(isInDirection(ab, ao)) {
            ...
            direction.times(dot(ao, direction));
        } else {
            size = 1;
            pointCase(ao); 
        }
    ...
    private boolean triangleCase(Vector2 ao, Vector2 ab, Vector2 ac) {
        ...
        axb.times(-dot(axb, ac));
        Vector2 axc = new Vector2(-ac.getY(), ac.getX());
        axc.times(-dot(axc, ab));
    ...
    public Vector2 getSupporting(Vector2 inDir) {
        ...
        Vector2 result = new Vector2(shape1[ia]);
        Vector2 minusDirection = new Vector2(inDir, Vector.NEGATE_BIT);
