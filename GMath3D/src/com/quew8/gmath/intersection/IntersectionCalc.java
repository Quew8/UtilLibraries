...
public class IntersectionCalc {
    ...
    public boolean intersects() {
        ...
        direction = a.negate(new Vector()); //Towards the origin
        int loopCount = 0;
        while(true) {
            loopCount++;
            if(loopCount > 200) {
    ...
    private boolean triangleCase(Vector ao, Vector ab, Vector ac, Vector abc) {
        ...
        if(abc.isMagnitudeZero()) {
            return onIntersection2Simplex(a, b, c);
        }
        //System.out.println("abc = " + abc);
        Vector abcac = cross(abc, ac);
        abcac.scale(abcac, -dot(abcac, ab));
        Vector ababc = cross(ab, abc);
        ababc.scale(abcac, -dot(ababc, ac));
    ...
    private boolean tetrahedronCase(Vector ao, Vector ab, Vector ac, Vector ad) {
        ...
        Vector abc = cross(ab, ac);
        abc.scale(abc, -dot(abc, ad));
        Vector abd = cross(ab, ad);
        abd.scale(abd, -dot(abd, ac));
        Vector acd = cross(ac, ad);
        acd.scale(acd, -dot(acd, ab));
